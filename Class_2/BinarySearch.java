...
public class BinarySearch {              // Time: O(1) | Space: O(1)
    ...
    public static int binarySearch(int[] arr, int target) {
        ...
        while (left <= right) {          // Time: O(log n) | Space: O(1)
            int mid = left + (right - left) / 2; // Time: O(1) | Space: O(1)
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Enter 5 elements:"); // Time: O(1) | Space: O(1)

        for (int i = 0; i < arr.length; i++) { // Time: O(n) | Space: O(1)
            arr[i] = in.nextInt();             // Time: O(1) | Space: O(1)
        }

        Arrays.sort(arr);                       // Time: O(n log n) | Space: O(log n)
        System.out.println("Sorted Array: " + Arrays.toString(arr)); // Time: O(n) | Space: O(1)

        System.out.print("Enter the target element to search: "); // Time: O(1) | Space: O(1)
        int target = in.nextInt();                                 // Time: O(1) | Space: O(1)

        int result = binarySearch(arr, target); // Time: O(log n) | Space: O(1)

        if (result != -1) {                     // Time: O(1) | Space: O(1)
            System.out.println("Element found at index: " + result); // Time: O(1) | Space: O(1)
        } else {
            System.out.println("Element not found in the array.");   // Time: O(1) | Space: O(1)
        }
